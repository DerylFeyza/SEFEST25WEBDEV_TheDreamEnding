...
export function Dashboard({ items, rentalHistory }: DashboardProps) {
  ...
  const completedRentals = rentalHistory.filter((rental) => rental.status === "COMPLETED")
    ...
    <Layout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-3 sm:col-span-3">
          <SustainabilitySection totalCarbonSavings={totalCarbonSavings} totalWasteReduction={totalWasteReduction} />
        </div>
      </div>
      <CallToAction />
      <UserEngagement />
